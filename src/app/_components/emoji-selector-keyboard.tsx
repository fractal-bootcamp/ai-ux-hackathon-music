...
export function EmojiSelector({
	...
	}, [onEmojiSelect, keyToEmojiMap, onBackspace, handlePageChange]);
					...
					>
						{row.map((key) => (
							<Button
								...
								onClick={() =>
									keyToEmojiMap[key] && onEmojiSelect(keyToEmojiMap[key])
								}
							>
								<span className="absolute top-1 left-1 text-xs opacity-30">
									{key}
								</span>
								{keyToEmojiMap[key] ?? ""}
							</Button>
